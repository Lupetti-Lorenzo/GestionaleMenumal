...
export async function handle({ event, resolve }) {
	...
	// proteggere admin routes e se non sono loggato - solo login unprotected
	if (!session && !unProtectedRoutes.includes(event.url.pathname)) throw redirect(303, "/login")
	...
	if (session) {
		...
		if (event.url.pathname === "/login") throw redirect(303, "/")
	}
