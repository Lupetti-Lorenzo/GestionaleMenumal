...
//      .../setStatoUser.php
    ...
    //     // $popUpStore.showPopUp("full", {jobName, dataRegistrazione, dbState})
    // }
    ...
    //     // $popUpStore.showPopUp("trial", {jobName, dataRegistrazione, dbState})
    // }

    const apriPopup = (event) => {
        const whichPopup = event.target.dataset.id
        popUpStore.showPopUp(whichPopup, {jobName, dataRegistrazione, dbState})
    }
