...
<div class="{loadingJobs ? "animate-pulse opacity-85": ""} bg-white shadow-md my-4">
    <!-- SEARCH FIELD -->
    <input disabled={loadingJobs} class="rounded-t focus-within:shadow-lg min-w-max w-full px-6 py-3  {loadingJobs ? "opacity-85": ""} bg-gray-200 text-gray-600 text-sm leading-normal" type="search" placeholder={loadingJobs ? "Caricamento...": "Search..."} bind:value={$jobsStore.search}/>
    <!-- TABLE OF JOBS -->
    <table class="min-w-max w-full table-auto">
        <thead>
            <tr class="{loadingJobs ? "opacity-85": ""} bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
